...
|---|---|---|
| `JANUS_EXCHANGE_SPINE` | `off` · **`record`** (default) · `authoritative` | `off`: no exchange rows written or read — zero behavior delta. `record`: the spine observes the dispatch/approval/observation seams and writes `agent_exchanges`/`exchange_events` rows; illegal transitions are logged, never thrown; **nothing reads exchange state to decide anything**. Also builds/stores instruction envelopes and scans pane output tails for a self-reported result envelope the agent printed on its own initiative (bounded: 8 KiB window / 25 candidates / strict zod / redaction+caps) — both observe-only. `authoritative`: the exchange row is the authoritative "what's in flight" — run-correlated dispatch join, boot-recovery quarantine surfacing, exchange-aware narration/SITREP/fleet; the rendered envelope IS the delivered instruction and resolver decisions govern routing (bind/confirm/clarify); the Workbench draft bridge is live. **Back-compat:** the pre-collapse spellings `shadow` and `primary` are still accepted and map to `record` / `authoritative` respectively. An unrecognized value falls through to the default (`record`) — safe because the default is the observe-only rung, and garbage can never reach `authoritative`. |
| `JANUS_AGENT_COMPLETION_PROMPT` | `off` · **`on`** (default) | The **only** knob in this subsystem that mutates outgoing agent-facing text — split out into its own clearly-named flag precisely so it cannot be enabled as a side effect of turning something else up. `on` (the **default** since the 2026-07-28 graduation; evidence gate: the bead-o2mf end-to-end smoke, `npm run smoke:completion-prompt`): the completion-request line — which the render profile appends regardless of this flag — gains the live exchange's own id in prose, so an agent that reports back has an id to echo (settlement becomes correlated rather than inferred from the legacy prose heuristic). `off`: delivered instructions are byte-identical — the independent opt-out. An unrecognized value falls through to the default (`on`) — a typo costs one benign prose hint line, never a behavior gate. Requires the spine to be on — **inert, never an error, when `JANUS_EXCHANGE_SPINE=off`**. Validation and settlement paths are identical whether or not the hint was sent. |
...
1. `JANUS_EXCHANGE_SPINE=record` — **this is now the DEFAULT (graduated from `off`, 2026-07); a
   fresh install starts here and stage 1 needs no action.** Collects lifecycle fidelity and
   populates the metrics/replay tables. No existing code path reads exchange state to make a
   decision in this mode, so behavior is unchanged; what you gain is the audit trail (and the
   evidence needed to justify stage 2). Set `JANUS_EXCHANGE_SPINE=off` to opt back out.
2. `JANUS_EXCHANGE_SPINE=authoritative` — exchange-aware notifications, recovery, fleet,
   ...
   **Promotion gate:** in `record` the target resolver runs as a DRY RUN — it logs the routing it
   *would* have chosen without steering delivery. Read `wrongTargetDeliveries` from
   `GET /api/exchange-metrics` (§3): non-zero means a resolver bug, and promoting would turn that
   bug into real mis-routed instructions. Promote when it reads zero across representative traffic.
3. `JANUS_AGENT_COMPLETION_PROMPT=on` — **this is now the DEFAULT (graduated from `off`,
   2026-07-28, on the strength of the end-to-end verification below); a fresh install starts here
   and stage 3 needs no action. Set `JANUS_AGENT_COMPLETION_PROMPT=off` to opt back out.**
   Additive settlement precision on top of the always-available conservative legacy heuristic.
