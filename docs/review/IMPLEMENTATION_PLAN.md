...
### WS-E — Spoken, targeted, safe approvals  *(M1, effort L)*  ✅ COMPLETE
...
> **STATUS: ✅ COMPLETE (`abf2508` + review-synthesis fixes).** All of E.1/E.2/E.3 landed.
> The least-authority gate runs on EVERY pane write (propose_command, execute_plan steps, and
> R4's plan-step ADVANCEMENT 2..N — the `activePlanGate` closes that bypass); `call.id` is
> answered exactly ONCE (non-blocking `pending_approval`; outcomes via `pushApprovalNarration`,
> never a 2nd `sendToolResponse`); the atomic `claim()` is the single mandatory gate inside one
> `resolveDecision`/`applyResolution` choke-point shared by REST approve, voice approve/reject,
> AND the TTL sweep (no path writes without winning the claim); the parser/targeting clarify
> rather than over-approve (bare weak verbs and fragment-misses → none/clarify); all
> spoken/stored/broadcast text is WS-B-redacted.
> **Follow-up:** BUG-040 (`POST /api/plans/:id/execute` writes step 0 un-gated) remains **OPEN**
> — it is the REST plan-execute endpoint, outside WS-E's voice scope; route it through the same
> `dispatchProposal` gate in a follow-up.
...
| 3 | WS-C (reliable status) | M1 | L | — (gates 4) ✅ DONE |
