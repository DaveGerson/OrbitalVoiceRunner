...
/**
 * bead 1p84 + eoef: assert the battery leaves no repo-root sentinel artifact — EVERY sentinel is
 * fatal. A .janus.db* leak (DB contamination) or a .janus_settings.json leak (a server-booting
 * test constructing an OrchestratorManager without pinning JANUS_SETTINGS_PATH) FAILS the battery.
 * The settings sentinel was advisory-only until the eoef sweep pinned every known offender via
 * tests/helpers/settingsPath.ts pinSettingsPathToTmpdir(); with the tree clean, this gate only
 * ever shrinks — a new leak is a regression in the offending test, never a reason to soften the
 * gate. Snapshot before/after the WHOLE run (both legs, if a retry happened) so a leak from
 * either leg is caught. Returns the leaked sentinels (a non-empty return fails the battery).
 */
function assertRepoRootCleanliness(before) {
  const after = snapshotRepoRootSentinels();
  const fatal = detectRepoRootLeak(before, after);
  if (fatal.length > 0) {
    console.error(
      `\n[run-unit] REPO-ROOT LEAK (fatal): the unit battery created ${fatal.join(", ")} at ` +
        ...
        `JanusStore/OrchestratorManager) without rooting its artifacts in a tmpdir — fix the ` +
        `offending test rather than deleting the file and moving on. Patterns to copy: ` +
        `tests/helpers/mockLive.ts loadMockLive() (DB rooting), tests/helpers/settingsPath.ts ` +
        `pinSettingsPathToTmpdir() (settings pinning).`,
    );
  }
  return fatal;
}
