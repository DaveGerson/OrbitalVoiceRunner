...
// ── Pure non-JSX helpers (exported for characterization tests) ────────────────

/**
 * Resolve which pane (if any) the Rulebook is currently scoped to.
 * Returns undefined when scope is "kitchen" or when the scoped pane is not found.
 */
export function resolveGateScope(
  scope: string,
  panes: RulebookPane[],
): RulebookPane | undefined {
  return scope !== "kitchen" ? panes.find((p) => p.id === scope) : undefined;
}

/**
 * Build the effective capability gate map for the current scope.
 * When a pane is scoped, its overrides win per-capability; unset keys inherit the global value.
 */
export function resolveGates(
  globalGates: CapabilityGateMap,
  scopedPane: RulebookPane | undefined,
): CapabilityGateMap {
  return scopedPane ? { ...globalGates, ...(scopedPane.overrides ?? {}) } : globalGates;
}

/**
 * The volume percentage to DISPLAY in the label (clamped to 0-100 regardless of
 * whether the stored value is in the 0-1 range or the 0-100 range).
 */
export function formatVolumePercent(volume: number): number {
  return Math.round((volume ?? 1) * (volume > 1 ? 1 : 100));
}

/**
 * The value to pass to the range <input> (always 0-100).
 * Volumes stored > 1 are already in the 0-100 range; values ≤ 1 are fractional.
 */
export function formatVolumeSlider(volume: number): number {
  return Math.round(volume > 1 ? volume : volume * 100);
}

/**
 * Returns true when the given service mode should render as active/selected.
 * "Inherit" collapses to "Human-in-the-Loop" for display purposes.
 */
export function isServiceModeActive(
  globalMode: "Full Auto" | "Human-in-the-Loop" | "Read-Only" | "Inherit",
  modeTarget: "Full Auto" | "Human-in-the-Loop" | "Read-Only",
): boolean {
  return (globalMode === "Inherit" ? "Human-in-the-Loop" : globalMode) === modeTarget;
}
...
export function BackOfHouse({ dark, settings, globalMode, setGlobalMode, saveSettings, panes = [], setPaneGates }: {
  ...
  const gates: CapabilityGateMap = resolveGates(globalGates, scopedPane);
  ...
  function renderRoomNav(): ReactNode {
    return (
  ...
  function renderRulebookRoom(): ReactNode {
    return (
  ...
  function renderChefRoom(s: SystemSettings): ReactNode {
    return (
      <Room emoji="👨‍🍳" title="Chef de Cuisine" sub="the global default — pick a single station in the Rulebook's scope row to tune just that pane" color="#ff8a3d">
        <Card dark={dark}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {SERVICE_MODES.map((m) => {
              const on = isServiceModeActive(globalMode, m.mode);
              return (
                <Fragment key={m.id}>
  ...
  function renderWalkinRoom(s: SystemSettings): ReactNode {
    return (
      <Room emoji="🧊" title="The Walk-In" sub="the only key Orbital keeps cold — your coding CLIs sign in themselves" color="#4db892">
        <Card dark={dark}>
          ...
          <WalkInKey dark={dark} masked={s.secrets.geminiApiKey} onSave={(key) => saveSettings({ ...s, secrets: { ...s.secrets, geminiApiKey: key } })} />
        </Card>
      </Room>
    );
  }

  function renderBoilerRoom(s: SystemSettings): ReactNode {
    return (
          ...
          {([
            ...
            <Fragment key={key}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                ...
                <input data-testid={`boiler-${key}`} type="number" defaultValue={s.advanced[key] ?? undefined}
  ...
  function renderRecipeRoom(s: SystemSettings): ReactNode {
    return (
  ...
  function renderRoomBody(): ReactNode {
    if (!settings) return <div style={{ fontFamily: "DM Sans", fontSize: 14, fontWeight: 600, color: "#8a6a4f" }}>loading the house…</div>;
    if (room === "rulebook") return renderRulebookRoom();
    if (room === "chef") return renderChefRoom(settings);
    if (room === "walkin") return renderWalkinRoom(settings);
    if (room === "boiler") return renderBoilerRoom(settings);
    if (room === "recipe") return renderRecipeRoom(settings);
    if (room === "dock") return <DisabledRoom dark={dark} emoji="📦" title="The Loading Dock" sub="integrations — GitHub, Jira, Linear" blurb="No deliveries wired up yet. When integrations land, you'll hook them up here." />;
    return <DisabledRoom dark={dark} emoji="💰" title="Boss's Office" sub="the budget" blurb="Spend tracking isn't plumbed yet. The numbers go here when the books are open." />;
  }
