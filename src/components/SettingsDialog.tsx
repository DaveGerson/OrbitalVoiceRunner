...
function parsePresetsSafe(input: any): CliPreset[] {
  if (Array.isArray(input)) {
    ...
    return input.map((item: any) => preservePresetGates(buildPresetFromArrayItem(item), item));
  }
  if (input && typeof input === "object") {
    // bead 8sq: carry per-preset capabilityGates through the rebuild (was being dropped on save).
    return Object.entries(input).map(([key, val]: [string, any]) =>
      preservePresetGates(buildPresetFromObjectEntry(key, val), val)
    );
  }
  return [];
}
...
export function SettingsDialog({
  ...
  useEffect(() => {
    apiFetch("/api/health")
      .then(res => res.json())
      .then((data: any) => {
        setMemorySynthStatus(coerceSynthStatus(data?.output?.memory?.synthesizer));
      })
      .catch(() => setMemorySynthStatus("unknown"));
  }, []);

  // Apply the resolved-default field groups (pure derivations live in settingsDialogHelpers) into
  // the component's local form state. Split out of the effect so the effect stays under the gate.
  const applyInitialSettings = (s: SystemSettings) => {
    const sv = deriveServerFields(s);
    setPort(sv.port);
    setHost(sv.host);
    setAppUrl(sv.appUrl);

    const v = deriveVoiceFields(s);
    setVoice(v.voice);
    setVoiceStyle(v.voiceStyle);
    setVolume(v.volume);
    setIsMicMuted(v.isMicMuted);
    setModel(v.model);
    ...
    setLocalWorkspacePath(pr.localWorkspacePath);

    setPresets(parsePresetsSafe(s.presets));

    if (s.announcements) {
      setAnnouncements(prev => mergeAnnouncements(prev, s.announcements));
    }

    const ad = deriveAdvancedFields(s);
    setMaxBufferLines(ad.maxBufferLines);
    setIdleTimeoutMs(ad.idleTimeoutMs);
    setAgentIdleTimeoutMs(ad.agentIdleTimeoutMs);
    setDefaultShellCommand(ad.defaultShellCommand);
    setGlobalPermissionsMode(ad.globalPermissionsMode);
    ...
    setGeminiApiKey(s.secrets?.geminiApiKey ?? "");

    // Format JSON string
    setRawJsonStr(JSON.stringify(s, null, 2));
  };
  ...
  // Generic: for each key present (`!== undefined`) in `patch`, invoke its matching setter. Folds the
  // repeated `if (patch.k !== undefined) setK(patch.k)` fan-out into one loop so the apply functions
  // stay under the gate. The patches only ever carry defined keys (built in the pure helpers).
  function applyPatch<P extends Record<string, any>>(
    patch: P,
    setters: { [K in keyof P]?: (v: NonNullable<P[K]>) => void }
  ) {
    for (const key of Object.keys(patch) as (keyof P)[]) {
      const value = patch[key];
      const setter = setters[key];
      if (value !== undefined && setter) setter(value as NonNullable<P[typeof key]>);
    }
  }

  // Apply a parsed JSON object into form state. The per-section value derivation (the `!== undefined`
  // guards, coercions, and the memorySynthTimeoutMs clamp) lives in pure helpers; here we only fan
  // the resulting partials out through the setters. Split out of handleJsonChange to stay under the gate.
  const applyParsedJson = (parsed: any) => {
    applyPatch(jsonServerPatch(parsed), { port: setPort, host: setHost, appUrl: setAppUrl });
    ...
    if (parsed.announcements) {
      setAnnouncements(prev => mergeAnnouncements(prev, parsed.announcements));
    }
    applyParsedJsonAdvanced(parsed);
    if (parsed.secrets) {
      if (parsed.secrets.geminiApiKey !== undefined) setGeminiApiKey(parsed.secrets.geminiApiKey);
    }
  };

  // Advanced section of the JSON-tab apply, including the gate normalization that the pure patch
  // intentionally excludes. Kept separate so applyParsedJson stays under the gate.
  const applyParsedJsonAdvanced = (parsed: any) => {
    // P0b memory synthesizer round-trip. The (n > 0 else 150) clamp lives in jsonAdvancedPatch.
    applyPatch(jsonAdvancedPatch(parsed), {
      maxBufferLines: setMaxBufferLines, idleTimeoutMs: setIdleTimeoutMs, agentIdleTimeoutMs: setAgentIdleTimeoutMs,
      defaultShellCommand: setDefaultShellCommand, globalPermissionsMode: setGlobalPermissionsMode,
      historyMaxCommands: setHistoryMaxCommands, historyMaxOutputLength: setHistoryMaxOutputLength,
      memoryPythonEnabled: setMemoryPythonEnabled, memorySynthTimeoutMs: setMemorySynthTimeoutMs,
    });
  ...
  // ─── Inline render helpers (in-body closures over component scope) ──────────────
  // These return ReactNode and are invoked inline as {renderX()} from the JSX below. They are NOT
  // React components (no <NewComp/>), so the fiber tree / hook identity is unchanged — they purely
  // relocate the tab/panel JSX (and its className ternaries) out of the SettingsDialog function body
  // to bring its McCabe complexity under the gate. JSX is byte-identical to the original inline form.
    ...
    <div className="flex flex-col lg:flex-row h-full overflow-hidden">
      ...
      <div className="w-full lg:w-48 bg-black/40 border-b lg:border-b-0 lg:border-r border-white/5 p-2 lg:p-4 flex flex-row lg:flex-col gap-1 shrink-0 font-mono text-[10px] uppercase tracking-wider overflow-x-auto lg:overflow-y-auto items-center lg:items-stretch">
        ...
        <button
          ...
          className={subTabClass(formTab === "gates")}
        >
        ...
        <button
          ...
          className={secretsTabClass(formTab === "secrets")}
        >
      ...
      <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-5">
        ...
        {formTab === "session" && renderSessionTab()}

        {/* 2. Project Profiles & Customizable CLI Startups list */}
        {formTab === "profiles" && renderProfilesTab()}
        ...
        {formTab === "advanced" && renderAdvancedTab()}

        {/* 4. Secrets Cabinet (Sensitive Key Cabinet) */}
        {formTab === "secrets" && renderSecretsTab()}

      </div>
    </div>
  );

  // Renders one preset card row. Extracted so the presets.map callback (and its status/footprint
  // derivations, already in pure helpers) stay flat; JSX byte-identical to the original inline card.
  const renderPresetCard = (preset: CliPreset): React.ReactNode => {
    const isDefault = ["claudeCode", "codex", "antigravity"].includes(preset.id);

    // Helper to identify terminals associated with this preset (pure predicate in helpers).
    const matched = terminals.filter(t => terminalMatchesPreset(t, preset));
    ...
    const { statusLabel, statusBadgeStyle, dotColor } = derivePresetStatus({
      numActive,
      hasAlert,
      anyRunning: matched.some(t => t.status === "Running"),
    });
      ...
      <div className="bg-[#0b0b0b] border border-white/10 rounded-xl w-full max-w-3xl h-[85vh] max-h-[800px] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex overflow-x-auto bg-black px-4 lg:px-6 border-b border-white/5 shrink-0 font-mono text-[11px] uppercase tracking-wide hide-scrollbar">
          ...
          <button 
            ...
            className={topTabClass(activeTab === "json")}
          >
            Interactive Raw JSON
          </button>
          <button 
            onClick={() => setActiveTab("install")}
            className={topTabClass(activeTab === "install")}
          >
