...
describe("c55 Batch A — server.ts cutover guard (no double-registration)", () => {
  ...
  // Isolate the mountRestRoutes(...) call so we assert membership in ITS `only` set specifically
  // (not just anywhere in the 3k-line file).
  const mountIdx = serverSrc.indexOf("mountRestRoutes(");
  // Slice from the call to the closing `});` of the only-set so membership is asserted against THIS
  // call's allow-set, not anywhere in the 3k-line file. Bound generously (the set + its comments are
  // well over a kilobyte) and clamp at the first `});` after the `only: new Set([` opener.
  ...
  for (const name of onlyNames) {
    it(`mountRestRoutes only-set includes "${name}"`, () => {
      ...
      assert.ok(
        ...
        `mountRestRoutes only-set must include "${name}" after the Batch-A cutover`
      );
    });
  }
